...
- Published updated amd64 and aarch64 images for release `1.0.138`.
- Rebuilt from latest `kingkiosk-core3` `origin/main` (`git.138`) and bundled latest `kingkiosk_admin` `origin/main` (`1.0.1+32`).
- Pushed architecture images to GHCR and kept both package repositories public for beta upgrades.

## [1.0.136] - 2026-02-18

### Changed

- Published updated amd64 and aarch64 images for release `1.0.136`.
- Reused the validated Intel amd64 artifact and rebuilt aarch64 locally from `kingkiosk-core3` `git.136`.
...
- Published updated amd64 and aarch64 images for release `1.0.131`.
- Synced release images to the latest `kingkiosk-core3` `origin/main` commit (`git.131`).
- Bundled the latest King Admin web build (`1.0.1+32`) into both architecture images.
...
- Published updated amd64 and aarch64 images for release `1.0.111`.
- Rebuilt King Admin web UI from latest local source and bundled it into both distributed architecture images.
